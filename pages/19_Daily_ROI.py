...
def render_content(fb_df, google_df, show_fb, show_google, date_from, date_to, key_prefix="dr"):
    """Render Daily ROI content. Can be called standalone or from a wrapper page."""
    # Summary comparison
    ...
    with tab1:
        if show_fb:
            ...
            st.download_button("📥 Download FB CSV", display_df.to_csv(index=False), f"fb_daily_roi_{datetime.now():%Y%m%d}.csv", key=f"{key_prefix}_dl_fb")
        else:
    ...
    with tab2:
        if show_google:
            ...
            display_df.columns = ['Date', 'Register', 'FTD', 'Recharge (PHP)', 'Cost (USD)', 'CPR', 'ROAS', 'Conv Rate %']
            st.dataframe(display_df, use_container_width=True, hide_index=True)
            st.download_button("📥 Download Google CSV", display_df.to_csv(index=False), f"google_daily_roi_{datetime.now():%Y%m%d}.csv", key=f"{key_prefix}_dl_g")
        else:
