...
def render_content(key_prefix="ra"):
    """Render Reporting Accuracy content. Can be called standalone or from Operations wrapper."""
    ...
    with fc3:
        ...
        agent_filter = st.selectbox("Select Agent", agents, key=f"{key_prefix}_agent")

    with fc4:
        st.markdown("**Scoring Rubric**")
        st.caption("4: <15min | 3: 15-24min | 2: 25-34min | 1: 35+min")
    ...
    with tab1:
        ...
        if agent_msgs.empty:
            ...
            if all_scores:
                ...
                st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}_chart_scores")
                ...
                st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}_chart_dist")
            else:
            ...
            if not filtered.empty:
                ...
                if not pivot.empty:
                    ...
                    fig = px.imshow(heatmap_data,
                                   ...
                                   aspect='auto')
                    fig.update_layout(height=400)
                    st.plotly_chart(fig, use_container_width=True, key=f"{key_prefix}_chart_heatmap")
        ...
        if not agent_msgs.empty:
            ...
            if not report_msgs.empty:
                ...
                st.download_button("📥 Download Report Messages CSV", csv,
                                  f"report_messages_{datetime.now():%Y%m%d}.csv", key=f"{key_prefix}_dl_reports")
            else:
    ...
    with tab4:
        ...
        if not agent_msgs.empty:
            ...
            st.dataframe(display, use_container_width=True, hide_index=True, height=500, key=f"{key_prefix}_tbl_all")

            csv = display.to_csv(index=False)
            st.download_button("📥 Download All Agent Messages CSV", csv,
                              f"agent_messages_{datetime.now():%Y%m%d}.csv", key=f"{key_prefix}_dl_all")
        else:
            st.info("No agent messages found.")
